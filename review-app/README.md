...
Use whatever keys your Merge actually outputs (`caption`, `text`, `url`, `media_id`, etc.). `personaKey` must be exactly one of: `linda_chambers` | `becca_rose` | `brooke_swift` | `abby` | `melissa_carter` | `claire_donovan` | `rebecca_lang` | `linda_ashford` | `eleanor_brody`.
...
| `{{ $json.personaKey }}` = `linda_ashford` | Linda Ashford Schedule Post |
| `{{ $json.personaKey }}` = `eleanor_brody` | Eleanor Brody Schedule Post |
